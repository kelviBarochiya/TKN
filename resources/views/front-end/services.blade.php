    ...
    <div class="container py-5">
        ...
        <div class="row g-4">
            @foreach($services as $index => $case)
                <div class="col-lg-4 wow fadeIn" data-wow-delay="{{ 0.3 + ($index * 0.2) }}s">
                    <div class="case-item position-relative overflow-hidden rounded mb-2">
                        <img class="img-fluid" src="{{ asset('public/images/' . $case->image) }}" alt="{{ strip_tags($case->type) }}">
                        <a class="case-overlay text-decoration-none" href="{{ route('front-end.single-service',[str_replace(' ','-',$case->slug)]) }}">
                            <small>{{ strip_tags($case->title) }}</small>
                            <h5 class="lh-base text-white mb-3">{{ strip_tags($case->title) }}</h5>
