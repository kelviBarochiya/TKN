...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            <a href="{{ route('meta_tag.create') }}" class="btn btn-primary" style="display:block;float:right;">Add Meta Tag</a>&nbsp;
            <a class="btn btn-danger" id="deleteDebtbtn" style="display:block;float:right;margin-right: 10px;">Delete</a>
      <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Meta Tag Tables</h4>
                ...
                @if(isset($metatags) && !empty($metatags))
                 @foreach ($metatags as $item)
                    @php
                        ...
                        if (strlen($item->meta_description) <= 20) {
                            $meta_description = $item->meta_description;
                        } else {
                            $meta_description = substr($item->meta_description, 0, 20) . '...';
                        }
                         if (strlen($item->meta_title) <= 20) {
                            $meta_title = $item->meta_title;
                        } else {
                            $meta_title = substr($item->meta_title, 0, 20) . '...';
                        }
                    ...
                    <tr>
                        ...
                        <div class="dropdown">
                            ...
                            <a class="dropdown-item" href="{{ route('edit.meta_tag',[$item->id]) }}"
                                ><i class="bx bx-edit-alt me-1"></i> Edit</a
                            >
                            <a class="dropdown-item" href="{{ route('delete.meta_tag',[$item->id]) }}"
            ...
            $(document).on('click', '#deleteDebtbtn', function(e) {
                ...
                if (jsAllSelectedIds == '' || jsAllSelectedIds == null) {
                    ...
                    if (confirm('Are you sure to delete this records?')) {
                        var ajax_url = "{{ route('delete_all_our_meta_tag') }}";
                        $.ajax({
                            headers: {
