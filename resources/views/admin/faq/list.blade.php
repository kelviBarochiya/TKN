...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            <a href="{{ route('faq.create') }}" class="btn btn-primary" style="display:block;float:right;">Add FAQ</a>&nbsp;
            <a class="btn btn-danger" id="deleteDebtbtn" style="display:block;float:right;margin-right: 10px;">Delete</a>
      <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Tables /</span> FAQ Tables</h4>
                ...
                @if(isset($faq) && !empty($faq))
                 @foreach ($faq as $item)
                    @php
                        ...
                        if (strlen($item->answer) <= 20) {
                            $answer = $item->answer;
                        } else {
                            $answer = substr($item->answer, 0, 20) . '...';
                        }
                    ...
                    <tr>
                        ...
                        <td>{!! html_entity_decode($answer) !!}</td>
                        ...
                        <div class="dropdown">
                            ...
                            <a class="dropdown-item" href="{{ route('edit.faq',[$item->id]) }}"
                                ><i class="bx bx-edit-alt me-1"></i> Edit</a
                            >
                            <a class="dropdown-item" href="{{ route('delete.faq',[$item->id]) }}"
            ...
            $(document).on('click', '#deleteDebtbtn', function(e) {
                ...
                if (jsAllSelectedIds == '' || jsAllSelectedIds == null) {
                    ...
                    if (confirm('Are you sure to delete this records?')) {
                        var ajax_url = "{{ route('delete_all_page') }}";
                        $.ajax({
                            headers: {
