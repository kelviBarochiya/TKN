...
class CheckAgentProjectPackage
{
    /**
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if(isset($getPackageName) && !empty($getPackageName)){
            ...
            if(isset($checkPackage)){
                $number_of_projects = $checkPackage->number_of_projects ?? "";

                $getProject = \App\Models\Project::where('created_by',$user->id)->count();

                if($number_of_projects == 0 || $number_of_projects == "0"){
                    return redirect()->route('front-end.packages'); 
                }else{
                    if($number_of_projects > $getProject){
                        return $next($request);
                    }else{
                        return redirect()->route('front-end.packages'); 
                    }
